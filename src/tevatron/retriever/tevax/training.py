...
def retriever_train_step(state, queries, passages, dropout_rng, axis="device"):
    ...
    def compute_loss(params):
        q_reps = state.apply_fn(
            **queries, params=params.q_params, dropout_rng=q_dropout_rng, train=True
        )[0][:, 0, :]
        p_reps = state.apply_fn(
            **passages, params=params.p_params, dropout_rng=p_dropout_rng, train=True
        )[0][:, 0, :]
...
def grad_cache_train_step(
    state, queries, passages, dropout_rng, axis="device", q_n_subbatch=1, p_n_subbatch=1
):
    try:
        from grad_cache import cachex
    except ImportError:
        raise ModuleNotFoundError(
            "GradCache packaged needs to be installed for running grad_cache_train_step"
        )
    ...
    queries, passages = cachex.tree_chunk(queries, q_n_subbatch), cachex.tree_chunk(
        passages, p_n_subbatch
    )
    ...
    q_reps = cachex.chunk_encode(partial(encode_query, state.params))(
        **queries, dropout_rng=q_rngs
    )
    p_reps = cachex.chunk_encode(partial(encode_passage, state.params))(
    ...
    loss, (q_grads, p_grads) = jax.value_and_grad(compute_loss, argnums=(0, 1))(
        q_reps, p_reps
    )

    grads = jax.tree_map(lambda v: jnp.zeros_like(v), state.params)
    grads = cachex.cache_grad(encode_query)(
        state.params, grads, q_grads, **queries, dropout_rng=q_rngs
    )
    grads = cachex.cache_grad(encode_passage)(
        state.params, grads, p_grads, **passages, dropout_rng=p_rngs
    )
