...
def decay_mask_fn(params):
    ...
    flat_mask = {
        path: (path[-1] != "bias" and "layernorm" not in path[-2])
...
class ModelArgs:
    ...
    def __post_init__(self):
        if self.model_type not in ENCODER_MODEL_MAPPING:
            raise ValueError(
                f"model_type must be one of {list(ENCODER_MODEL_MAPPING.keys())}"
            )
...
def main():
    ...
    if train_args.train_file.endswith(".jsonl"):
        train_data = datasets.load_dataset("json", data_files=train_args.train_file)[
            "train"
        ]
    else:
    ...
    if tokenizer.pad_token_id is None:
        logger.warning(
            "Tokenizer does not have a pad token. Adding eos token as pad token"
        )
        tokenizer.pad_token = tokenizer.eos_token
    train_dataset = TrainDataset(
        train_data,
        train_args.num_target_passages,
        tokenizer,
    ...
    # optimizer setup
    total_train_steps = (
        len(train_dataset) // train_args.batch_size * train_args.num_epochs
    )
    ...
    optimizer = optax.chain(
        optax.clip_by_global_norm(train_args.max_grad_norm), optimizer
    )
    ...
    checkpoint_manager = magix.get_chckpoint_manager(
        train_args.checkpoint_dir, train_args.save_steps, items=["lora", "optimizer"]
    )
    ...
    model, params = magix.load_model_hub(
        _model_cls, model_args.model_name, model_sharding_config, mesh, half=True
    )
    ...
    lora_shapes, opt_shapes = jax.eval_shape(
        create_lora_and_opt_states, lora_rng, params
    )
    lora_sharding = magix.lora.create_lora_sharding(
        model_sharding_config, mesh, lora_shapes
    )
    opt_sharding = magix.lora.create_lora_sharding(sharding_config, mesh, opt_shapes)

    if is_new_train:
        lora_params = jax.jit(lora.init_params, out_shardings=lora_sharding)(
            lora_rng, params
        )
    ...
    def train_step_cached(
        ...
        loss, grads = jax.value_and_grad(compute_loss, argnums=0)(
            lora_params, params, queries, passages
        )
        metrics = {"loss": loss}
    ...
    def combine_metrics(list_of_dicts):
        return {
            key: jnp.array([d[key] for d in list_of_dicts]) for key in list_of_dicts[0]
        }

    epochs = tqdm(
        range(train_args.num_epochs),
    ...
    with mesh:
        for epoch in epochs:
            ...
            batch_loader = Batches(
                input_rng, train_dataset, train_args.batch_size, epoch, shuffle=True
            )
            ...
            for step in trange(steps_per_epoch):
                ...
                lora_params, opt_state, metrics = p_train_step(
                    params, lora_params, opt_state, qq, pp, dropout_rngs
                )
            ...
            epochs.write(f"Epoch... ({epoch + 1}/{train_args.num_epochs})")
