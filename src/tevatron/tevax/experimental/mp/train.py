...
def main():
    ...
    checkpoint_manager = magix.get_chckpoint_manager(
        train_args.checkpoint_dir, train_args.save_steps
    )
    ...
    if is_new_train:
        ...
        model, params = magix.load_model_hub(
            _model_cls, model_args.model_name, sharding_config, mesh
        )
    ...
    with mesh:
        for epoch in epochs:
            ...
            for step in trange(steps_per_epoch):
                ...
                params, opt_state, metrics = p_train_step(
                    params, opt_state, qq, pp, dropout_rngs
                )

                is_last_step = (cur_step + 1) == total_train_steps
                checkpoint_manager.save(
                    cur_step,
                    items={"model": params, "optimizer": opt_state},
                    force=is_last_step,
                )
