...
def contrastive_loss_2dm(ss, tt, scale_by_dim=False):
    ...
    @partial(
        ...
        out_specs=PS(None, DATA_AXIS)
    )
    def sim(ss, tt):
        dp_size = lax.psum(1, DATA_AXIS)
        dp_idx = lax.axis_index(DATA_AXIS)
        acc = jnp.zeros((ss.shape[0]*dp_size, tt.shape[0]), dtype=jnp.float32)
        for op_idx in range(0, dp_size - 1):
            ...
            update_idx = (op_idx + dp_idx) % dp_size
            acc = lax.dynamic_update_slice(acc, sim_chunk, (update_idx*ss.shape[0], 0))
        # last chunk
        sim_chunk = jnp.einsum('si,ti->st', ss, tt, preferred_element_type=jnp.float32)
        sim_chunk = lax.psum(sim_chunk, MODEL_AXIS)
        update_idx = (dp_size - 1 + dp_idx) % dp_size
        acc = lax.dynamic_update_slice(acc, sim_chunk, (update_idx*ss.shape[0], 0))
