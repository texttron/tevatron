...
def transform_dataset(ds, task):
    ...
    trans_ds = ds.add_column("image", new_image_column).add_column(
        "source", new_source_column
    )
...
def upload_dataset(new_ds_dict, task):
    new_ds_dict.push_to_hub(
        "ArvinZhuang/bright-corpus", config_name=task, split="train"
    )
