...
class BasicHardNegativeMiner:
    def __init__(self, results_path, corpus_dataset, depth):
        ...
        self.docid_to_idx = manager.dict(
            {k: v for v, k in enumerate(self.corpus_data["docid"])}
        )
...
if __name__ == "__main__":
    ...
    combined_data = combined_data.filter(
        function=lambda data: len(data["positive_passages"]) >= 1
        and len(data["negative_passages"]) >= args.min_hn
    )
